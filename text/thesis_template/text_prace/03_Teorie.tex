...
\citet{faltejskovaNonconsensusFlankingSequence2026a} addressed this question in a recent descriptive analysis (preprint).
...
Mechanistic interpretability analyses, using sparse autoencoders trained on Evo~2
representations, identified internal features associated with transcription factor binding
motifs in human promoter sequences: unsupervised SAE features matched 70\% of
promoter-enriched motifs from the HOCOMOCO database, compared with 35\% recalled
by HOMER, a dedicated motif discovery algorithm \cite{brixiGenomeModellingDesign2026}.
These analyses characterise motif-level representations within and immediately around
binding sites.
Whether Evo~2 embeddings also encode the kilobase-scale compositional gradients that
extend over 1--2~kb on either side of in vivo binding sites — as described by
\citet{faltejskovaNonconsensusFlankingSequence2026a} — has not been examined,
and constitutes the empirical question taken up in this thesis.
