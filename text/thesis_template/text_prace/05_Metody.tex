...
To verify that the magnitude elevation observed at the left edge of the
forward positional profiles reflects the model's autoregressive processing
direction rather than an intrinsic sequence property of the window edges, one BED file
(ENCFF765CKK, MYC GM12878, $n = 2{,}203$ peaks after N-bin filtering) was
re-embedded with all sequences replaced by their reverse complements.
Reverse complementation was applied to each extracted 10\,kb sequence prior to
tokenisation; all other embedding parameters were identical to the forward pass
(checkpoint \texttt{evo2\_7b}, layer \texttt{blocks.28.mlp.l3}, 200 bins of
50\,bp, BOS token prepended and discarded).
The forward file's N-bin drop mask was applied to the RC embedding array before
comparison, ensuring row correspondence between the two passes.
Before comparison, the RC positional profile was flipped along the bin axis to
genomic coordinates, since model position~0 of the RC pass corresponds to
genomic bin~199 of the original window.
Warm-up elevation was quantified as the ratio of mean L2 magnitude in the
expected warm-up region (bins~0--14 for the forward pass; bins~185--199 for the
RC pass) to mean magnitude in the remaining bins.
